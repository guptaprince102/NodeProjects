...
export class Orders{
    ...
    page : Page;
    orderRowsLocator : Locator;
    orderIdInsideOrderRowLocator : Locator;
    ...
    async goToOrderDetail(orderId : string){
        ...
        for(let i = 0 ; i < await this.orderRowsLocator.count() ; i++){
            const orderIdInsideOrderRow : string = (await this.orderIdInsideOrderRowLocator.nth(i).textContent()) ?? "";
